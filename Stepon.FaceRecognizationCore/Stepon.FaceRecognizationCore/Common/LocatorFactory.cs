using Stepon.FaceRecognizationCore.Detection;
...
namespace Stepon.FaceRecognizationCore.Common
{
    ...
    public class LocatorFactory
    {
        ...
        /// <param name="maxFaceNumber">用户期望引擎最多能检测出的人脸数有效值范围[1,50]</param>
        /// <returns></returns>
        public static FaceLocator GetDetectionLocator(string appId, string sdkKey, FaceExtra age, FaceExtra gender, int preAllocMemSize = 41943040,
            OrientPriority orientPriority = OrientPriority.OrientHigherExt,
            int scale = 16, int maxFaceNumber = 10)
        {
            return GetLocator(appId, sdkKey, age, gender, false, preAllocMemSize, orientPriority, scale, maxFaceNumber);
        }
